          ...
          <section class="wrapper">
			...
			<thead>
				...
				<th><i class=" fa fa-edit"></i> {{trans('User Status')}}</th>
                                <th style="text-align: center;" colspan="4"><i class="fa fa-cogs"></i> {{trans('User Options')}}</th>
			</thead>
			<tbody>
						...
						<td>
							...
							@else
								<button style="margin-right:5%" class="btn btn-theme04" type="button" onClick="location.href='{{ action('UserController@unban', array($user->id)) }}'"><i class="fa fa-bullhorn"></i>  {{trans('Unban')}}</button>                                                 
                                                                                                ...
                                                                                                <td>
							@endif
							<button class="btn btn-danger" href="{{ action('UserController@destroy', array($user->id)) }}" data-token="{{ Session::getToken() }}" data-method="delete"><i class="fa fa-trash-o "></i>  {{trans('Delete')}}</button>
                                </td>
					</tr>                        
				@endforeach 
			</tbody>
